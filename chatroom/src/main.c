...
#include "../common/my_friends.h"
#include "../common/my_deal.h"
#include "../common/my_mysql.h"
#include "../common/my_socket.h"
#include "../common/my_err.h"
#include "../common/my_pack.h"
        ...
        for (i = 0; i < nfds; i++) {
            if (events[i].data.fd == sock_fd) { // 有客户端连接请求
                ...
                if (recv_pack.type == EXIT) {
                    ...
                    epoll_ctl(kdpfd, EPOLL_CTL_DEL, events[i].data.fd, &ev); // 将该客户端fd从红黑树中摘下
                    curfds--;
